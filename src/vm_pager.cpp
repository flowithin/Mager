 ...
 *  @note when this function is run, there is no free page
 *
 * */
int pm_evict() {
  assert(free_ppage.empty());
    ...
    assert(_it != infile.end());//should exist from vm_map
...
int alloc() {
    ...
    // otherwise evict to make space
        ...
        ppage = pm_evict();
        if (ppage == -1) return -1;//shouldn't be 
    }
    return ppage;
}
...
bool is_cow(page_table_entry_t* pte) {
  ...
  // 6-credit
    return (core.find(pte->ppage) != core.end() && core[pte->ppage].size() > 1 && infile[pte].ftype == file_t::SWAP
            && !infile[pte].infile)
        || pte->ppage == pinned;//only needed for 4-credit
}
